...
pub(super) fn materials(
    ams: Option<&AmsState>,
    external_tray: Option<&Tray>,
    active_tray: Option<i64>,
) -> Vec<Material> {
    let mut materials = ams_materials(ams, active_tray);
    if let Some(external) = external_material(external_tray, active_tray) {
        materials.push(external);
    }
    materials
}

fn ams_materials(ams: Option<&AmsState>, active_tray: Option<i64>) -> Vec<Material> {
    ...
    for (ams_index, ams_unit) in ams.ams.iter().enumerate() {
        for (tray_index, tray) in ams_unit.tray.iter().enumerate() {
            ...
            if let Some(material) = material_summary(tray, label, active) {
...
fn external_material(tray: Option<&Tray>, active_tray: Option<i64>) -> Option<Material> {
    tray.and_then(|tray| {
        let active =
            active_tray.is_some_and(|active| tray.id.is_some_and(|tray_id| active == tray_id));
        material_summary(tray, "ext".to_owned(), active)
    })
}

fn material_summary(tray: &Tray, label: String, active: bool) -> Option<Material> {
    let kind = tray
        .material
        .clone()
