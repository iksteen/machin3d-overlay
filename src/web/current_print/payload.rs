...
pub(super) fn overlay_payload(
    ...
    OverlayPayload {
        ...
        mqtt,
        devices: devices.into_iter().map(device_payload).collect(),
    }
}

pub(super) fn error_payload(error: impl Into<String>, mqtt: MqttStatusPayload) -> ErrorPayload {
    ErrorPayload {
        ok: false,
        error: error.into(),
        updated_at: Utc::now().to_rfc3339(),
...
fn device_payload(device: DeviceSummary) -> DevicePayload {
