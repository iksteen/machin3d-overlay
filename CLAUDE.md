...
Runtime entry points come from the `bambu-overlay` CLI: `login`, `devices`, `mqtt`, `serve`. See `README.md` for end-user invocation patterns; that file is the source of truth for user-visible behavior such as `--bbl-cloud-device` / `--bbl-local-device` / `--bbl-video-device` / `--snap-device` semantics and `/bind` enumeration rules.
