...
- Don't serialize access codes into web responses; route credentials through `DeviceEntry::access_code()` accessors and keep them out of `/api/devices` and similar payloads.
- Device IDs come from TLS certificate CNs for local printers — do not bypass the CN check on either the MQTT or video path.
