...
4. **Background subsystems** (spawned by `BackgroundServices::spawn` against a single `Shutdown` broadcast):
   ...
   - **Moonraker workers** (`src/moonraker/backend.rs`) — one WebSocket session per Moonraker device (Snapmaker U1 today, any conformant Moonraker/Klipper printer in principle), subscribed to `print_stats`, `display_status`, `extruder[N]`, `heater_bed`, `fan`, `virtual_sdcard`, `print_task_config`, `gcode_move`, and `toolhead`. `notify_status_update` events are merged into a status map and converted to vendor-neutral `PrinterReport`s by `moonraker/report.rs`. Klipper reports no ETA of its own, so `report.rs` derives the remaining time from `print_stats.print_duration` over the progress fraction (`display_status.progress`, i.e. the slicer's `M73`, falling back to `virtual_sdcard.progress`), withheld below 1% progress where the extrapolation is nonsense. That total is re-derived **only when progress moves** (`EtaTracker`, one per session): `M73` steps minutes apart, and recomputing between steps makes the remaining time count *up* at several times real speed. Total print time, filament weight and start time come instead from the slicer metadata (`moonraker/metadata.rs` → `server/files/metadata`, fetched once per job by `backend.rs`'s `JobFacts`, retried every 30s while Moonraker has not parsed the file yet) — `estimated_time` is a constant for the job, so it does not wobble. That same module owns the metadata fetch the thumbnail path uses for embedded previews. The WS client itself is `moonraker/client.rs`; Snapmaker-U1-only material (mTLS pairing, the camera control plane) is quarantined under `moonraker/u1/`. Both vendor backends publish into the shared `LiveStateStore`.
   - **VideoStreams** (`src/video/`) — at most one upstream connection per printer. Browser/OBS clients on the same `/devices/<id>/video.mjpeg` share frames via `broadcast`; the upstream closes after the last client disconnects. Per-vendor sources (`BambuVideoSource` / `MoonrakerVideoSource`) decide how frames arrive: Bambu reads an authenticated TLS stream of raw JPEG frames; Moonraker polls the camera-proxy JPEG (generic, in `video/moonraker.rs`) and drives the Snapmaker U1's camera daemon with `camera.start_monitor` / `camera.stop_monitor` — over a long-lived mTLS MQTT session on paired printers, otherwise over Moonraker's unauthenticated `camera.*` WebSocket repeater (`video/u1_camera.rs` — the only Snapmaker-specific part of the video path). Bambu TLS uses `native-tls` with **only Bambu's BBL CA** trusted and hostname verification disabled — instead, after handshake the code matches the cert CN to the requested device ID before sending the camera access code. Successful Bambu endpoints are remembered for the process lifetime; mismatched endpoint/device pairs are also remembered to skip on retry.
...
- The Snapmaker U1 camera daemon only writes fresh frames to `monitor.jpg` while in monitor mode, and its watchdog disarms monitor mode ~361s after the last `camera.start_monitor` — so the video worker re-sends it every `u1_camera::HEARTBEAT` (120s) for as long as a client is attached, and `camera.stop_monitor`s on the way out. Two transports reach the daemon and both are live: paired devices use the mTLS MQTT control plane (`domain: "lan"`, `expect_pw: true`), everything else falls back to Moonraker's `camera.*` JSON-RPC repeater over `ws://HOST/websocket`, which needs no credentials because the U1 keeps the private ranges in `trusted_clients`. The repeater is WebSocket-only (the HTTP transport for those methods is disabled on-device), the method name is dotted (`camera.start_monitor`; a `camera/…` path 404s), and it is fire-and-forget — Moonraker answers `result: null` and the daemon's real reply goes to the MQTT `camera/response` topic. That costs nothing: the HTTP poll URL is the legacy `/server/files/camera/monitor.jpg?_nocache=<unix_ms>` path, not what the daemon's `start_monitor` response advertises (that's an internal moonraker mount path). `U1_WEBCAM_PROTOCOL.md` documents the whole protocol.
